...
class WWTPDataPipeline:
    ...
    def __init__(
        ...
        self.freq = freq or CFG.pipeline.freq
        self.lookback = lookback or CFG.pipeline.lookback
        self.horizon = horizon or CFG.pipeline.horizon
        self.max_interp_gap = max_interp_gap or CFG.pipeline.max_interp_gap
        self.test_ratio = test_ratio or CFG.pipeline.test_ratio
    ...
    def load_and_validate(self, filepath: Optional[str] = None) -> pd.DataFrame:
        ...
        # 🚀 【修复点 A】强制使用 utf-8-sig 编码，从根源消除 Windows Excel 生成的 BOM 字符

...
from models.lgbm_features import LGBMFeatureBuilder, FeatureConfig
...
class LGBMTrainEngine:
    ...
    def cross_validate(self, X: np.ndarray, y: np.ndarray, feature_cols: list,
                       n_folds: int = 5) -> dict:
        """Expanding-window time-series CV. Preserves temporal order.

        Returns dict with cv_mae_mean, cv_mae_std, cv_rmse_mean, cv_rmse_std, cv_folds.
        """
        n = len(X)
        min_required = n_folds * 10
        if n < min_required:
            self._log("WARN", f"Dataset too small for CV (n={n} < {min_required}), skipped")
            return {"cv_mae_mean": None, "cv_rmse_mean": None, "cv_folds": 0}

        self._log("INFO", f"--- Time-series {n_folds}-fold CV (Expanding Window) ---")
        self.progress_cb(5.0, f"Running {n_folds}-fold time-series CV...")
        ...
        for fold in range(n_folds):
            ...
            test_data = lgb.Dataset(X_te, label=y_te, feature_name=feature_cols, reference=train_data)

            model = lgb.train(
                clean_params, train_data,
                num_boost_round=self.train_cfg.n_estimators,
                valid_sets=[test_data],
                callbacks=[lgb.early_stopping(stopping_rounds=self.train_cfg.early_stop_rounds),
            ...
            if model is not None and model.current_iteration() > 0:
                ...
                self._log("INFO", f"  Fold {fold+1}/{n_folds}: MAE={mae_scores[-1]:.3f}, RMSE={rmse_scores[-1]:.3f}")

        if not mae_scores:
            return {"cv_mae_mean": None, "cv_rmse_mean": None, "cv_folds": 0}

        result = {"cv_mae_mean": float(np.mean(mae_scores)),
        ...
        self._log("SUCCESS",
                  f"CV {len(mae_scores)}-fold: MAE={result['cv_mae_mean']:.3f}+-{result['cv_mae_std']:.3f}, "
                  f"RMSE={result['cv_rmse_mean']:.3f}+-{result['cv_rmse_std']:.3f}")
        return result
    ...
    def train_and_save(self, lgbm_data: dict, save_dir: str) -> Dict[str, Any]:
        ...
        # --- Time-series cross-validation (optional, config-driven) ---
        cv_folds = getattr(getattr(CFG, 'training', None), 'lgbm_cv_folds', None)
        if cv_folds and cv_folds > 1:
            self.cross_validate(np.vstack([X_train_aligned, X_test_aligned]),
                              np.concatenate([y_train, y_test]),
                              feature_cols, n_folds=cv_folds)

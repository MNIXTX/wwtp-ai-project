...
if not IS_PACKAGED:
    if not check_virtual_environment():
        ...
        sys.stderr.write("   • PowerShell: .\\venv\\Scripts\\python.exe training/run_pipeline.py\n")
        sys.stderr.write("   • Conda: conda activate <your_env_name> && python training/run_pipeline.py\n")
        sys.stderr.write("\033[91m" + "="*60 + "\033[0m\n")
# ============================================================
...
def run_pipeline(
    ...
    logger.info("   1. 训练 LGBM/TFT 预测模型: python training/train_ppo.py --model lgbm_tft")
    logger.info("   2. 训练 PPO 控制策略:      python training/train_ppo.py --model ppo")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
示例:
  python training/run_pipeline.py                          # 使用 config.yaml 中的默认路径
  python training/run_pipeline.py --csv data/my_plant.csv  # 指定 CSV 文件（绕过 config.yaml）
  python training/run_pipeline.py --csv "D:/工厂数据/2024年/1月.csv"  # 支持绝对路径和中文路径
        """.strip(),
    )
    parser.add_argument(
        "--csv", dest="csv_path",

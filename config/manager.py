...
class ModelConfig(BaseModel):
    ...
    tft_seq_len: int = 24
    tft_num_features: int = 7
    tft_lstm_layers: int = 2
    tft_dropout: float = 0.15
    tft_num_attn_heads: int = 4       # 多头注意力头数 (新增)
    ...
    tft_feature_names: List[str] = ["inf_cod", "inf_nh3", "flow", "DO_reactor", "MLSS_reactor", "temp", "pH"]
...
class TrainingConfig(BaseModel):
    ...
    lgbm_early_stop_rounds: int = 100
    lgbm_log_every: int = 100
    lgbm_cv_folds: int = 5            # 时间序列交叉验证折数 (新增)
    lgbm_cv_gap: int = 0              # CV 折间间隔 (0=expanding window)

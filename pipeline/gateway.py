...
from models.lgbm import validate_dual_model_consensus
...
def _infer_lgbm_base_columns_from_model(lgbm_model) -> Optional[List[str]]:
    ...
    try:
        ...
        for name in all_names:
            # 去掉所有特征工程后缀 (lag, rolling, VSL, delta, interaction, cyclical)
            root = re.sub(r'_(lag|roll_mean|roll_std|roll_range|roll_slope|roll_entropy|roll_crossrate|delta_1h|x_.*)_\d+$', '', name)
            root = re.sub(r'_(lag|roll_mean|roll_std|roll_range|roll_slope|roll_entropy|roll_crossrate|delta_1h)$', '', root)
            # 去掉交互特征后缀 (e.g. flow_x_inf_cod → root is one of the pair)
            root = re.sub(r'_x_.*$', '', root)
            if root not in seen and root not in ('hour', 'dayofweek', 'hour_sin', 'hour_cos', 'dow_sin', 'dow_cos', 'month_sin', 'month_cos'):
                seen.add(root)
                base.append(root)
        if len(base) >= 4:
...
class WaterQualityPredictor:
    ...
    def __init__(
        ...
        # Override expected columns with exact training-time feature list
        import joblib as _jb
        cols_path = Path(CFG.paths.lgbm_model_dir) / "lgbm_feature_cols.joblib"
        if cols_path.exists():
            try:
                saved_cols = _jb.load(cols_path)
                if saved_cols and len(saved_cols) == self.lgbm_model.num_feature():
                    self._lgbm_builder._expected_columns = list(saved_cols)
                    logger.info(f"LGBM columns loaded from training artifacts: {len(saved_cols)} columns")
            except Exception:
                pass
    ...
    def predict(self, history_data: Dict[str, List[float]]) -> Dict:
        ...
        try:
            ...
            result["warnings"].append(
                ...
                f"建议重新训练: python training/train_tft.py）: {err_msg}"
            )

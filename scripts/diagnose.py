...
用法: python scripts/diagnose.py
     或在项目根目录: python scripts/diagnose.py
"""
...
def check_config():
    ...
    if cfg.exists():
        ...
        print(f"  预期位置: {cfg}")
        print("  修复: 从备份恢复或运行 python config_manager.py 生成默认配置")
    ...
    if tft.exists():
        ...
        if size > 1000:
            ...
            if magic == b'\x08':
                ...
                print(f"  修复: 重新训练 TFT 模型 python train_tft.py")
        elif size > 0:
    ...
    if lgbm.exists():
        ...
        if size > 100:
            ...
            if header.startswith(b'tree'):
                ...
                print(f"  修复: 重新训练 LGBM 模型 python run_lgbm.py")
        elif size > 0:
...
def main():
    ...
    if results["fail"] > 0:
        ...
        print("  2. 恢复配置文件: 从备份复制 config.yaml")
        print("  3. 重新训练模型: python run_pipeline.py && python run_lgbm.py")
        print("  4. 清理进程:     运行 stop.bat")
        return 1
    else:
